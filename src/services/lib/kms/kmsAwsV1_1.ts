...
): Promise<[KMSClient, string, string]> {
  const credProvider = fromCognitoIdentityPool({
    identityPoolId: identityPoolRoute.resourceUri,
    logins: {
      'securetoken.google.com/bright-zodiac-339920': idToken, //securetoken.google.com/#firebaseID
    },
...
export class KMSAwsV1_1 implements KMS {
  constructor(
  ...
  static async getIdentityPoolRoute(): Promise<AWSRoutableResource> {
    return DEV_IDENTITY_POOL_ROUTE
    // return !isLive()
    //   ? DEV_IDENTITY_POOL_ROUTE
    //   : ((await locationRoutingController.route(
  ...
  static async getKMSRoute(): Promise<AWSRoutableResource> {
    return (await locationRoutingController.route(
      KMS_ROUTES,
      KMS_ROUTES[0],
    )) as AWSRoutableResource;
  }
